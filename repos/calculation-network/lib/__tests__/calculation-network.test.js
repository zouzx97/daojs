...
describe('CalculationNetwork', () => {
  ...
  describe('#get()', () => {
    ...
    it('should be able to get orphan cells', async () => {
      await expect(cn.get('orphan')).resolves.toEqual(1);
    });
  ...
  describe('#addObserver()/#removeObserver', () => {
    ...
    it('should prevent duplicated observers', async () => {
      const onUpdate = jest.fn();
      ...
      expect(() => cn.set({ foo: 2 })).not.toThrow();
    });
  });
});
